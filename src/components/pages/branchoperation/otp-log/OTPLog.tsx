...
import { getOTPLogColumns } from "@/app/(dashboard)/branch-operations/otp-log/columns"
import { branchOpsService } from "@/services/branchops.service"
import { OTPLog as OTPLogType } from "@/types/branchops.types"
import { Button } from "@/components/ui/button"
import { ChevronLeft } from "lucide-react"
import PageHeader from "@/components/layout/PageHeader"
import { useLanguage } from "@/components/providers/LanguageProvider"

interface OTPLogProps {
  mobile: string
}

export default function OTPLog({ mobile }: OTPLogProps) {
    const router = useRouter()
    const { language } = useLanguage()
    
    const [data, setData] = useState<OTPLogType[]>([])
